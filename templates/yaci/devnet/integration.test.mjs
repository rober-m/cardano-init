...
async function getJson(path) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (!res.ok) fail(`Devnet request returned HTTP ${res.status}`);
      return res.json();
    } catch (err) {
      lastErr = err;
  ...
  if (requireDevnet) {
    fail(`Devnet required (CARDANO_INIT_REQUIRE_DEVNET) but unreachable (${lastErr?.code ?? lastErr?.name ?? "error"}).`);
  }
  skip(`Devnet not reachable (${lastErr?.code ?? lastErr?.name ?? "error"}).`);
