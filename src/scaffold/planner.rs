...
pub fn plan(selection: &Selection, registry: &Registry) -> Result<FilePlan, ScaffoldError> {
    ...
    for assignment in &selection.assignments {
        ...
        let manifest_data = TemplateAssets::get(&manifest_key).ok_or_else(|| {
            ScaffoldError::AssetNotFound {
                path: manifest_key.clone(),
            }
        })?;
        let manifest_text = std::str::from_utf8(&manifest_data.data)
            .expect("manifest.toml must be valid UTF-8");
